...
async def run(node_id):
    ...
    data_id = raftos_plus.Replicated(name="data_id")

    # Dict-like object: data.update(), data['key'] etc
    data = raftos_plus.ReplicatedDict(name="data")

    # List-like object: data_list.append(), data_list[-1]
    data_list = raftos_plus.ReplicatedList(name="data_list")

    while True:
        # We can also check if raftos.get_leader() == node_id
        await raftos_plus.wait_until_leader(node_id)
...
if __name__ == "__main__":
    ...
    loop.create_task(raftos_plus.register(node, cluster=cluster))
