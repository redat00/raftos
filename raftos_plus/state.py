...
from raftos_plus.exceptions import NotALeaderException
from raftos_plus.storage import FileStorage, Log, StateMachine
from raftos_plus.timer import Timer
...
def validate_term(func):
    ...
    def on_receive_function(self, data):
        ...
        if self.storage.term > data["term"] and not data["type"].endswith(
            "_response"
        ):
            response = {
...
def validate_commit_index(func):
    ...
    def wrapped(self, *args, **kwargs):
        for not_applied in range(
            self.log.last_applied + 1, self.log.commit_index + 1
        ):
...
class BaseState:
    ...
    def on_receive_append_entries(self, data):
        ...
        Receiver implementation:
            ...
            5. If leader_commit > commit_index,
...
class Leader(BaseState):
    ...
    - If last log index ≥ next_index for a follower: send AppendEntries RPC
    with log entries starting at next_index
    ...
    next_index and retry
    — If there exists an N such that N > commit_index, a majority
    of match_index[i] ≥ N, and log[N].term == self term: set commit_index = N
    """
    ...
    def init_log(self):
        self.log.next_index = {
            follower: self.log.last_log_index + 1
            for follower in self.state.cluster
        }
    ...
    async def append_entries(self, destination=None):
        ...
        for destination in destination_list:
            ...
            asyncio.ensure_future(
                self.state.send(data, destination), loop=self.loop
            )
    ...
    def on_receive_append_entries_response(self, data):
        ...
        # and step down via <step_down_timer> if leader doesn't get majority
        # of responses for <step_down_missed_heartbeats> heartbeats

        if data["request_id"] in self.response_map:
            self.response_map[data["request_id"]].add(sender_id)

            if self.state.is_majority(
                len(self.response_map[data["request_id"]]) + 1
            ):
                self.step_down_timer.reset()
                del self.response_map[data["request_id"]]

        if not data["success"]:
            self.log.next_index[sender_id] = max(
                self.log.next_index[sender_id] - 1, 1
            )
    ...
    def update_commit_index(self):
        ...
        for index in range(
            self.log.commit_index + 1, self.log.last_log_index + 1
        ):
            commited_count = len(
                [
                    1
            ...
            # If index is matched on at least half + self for current term —
            # commit
...
class Candidate(BaseState):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.election_timer = Timer(
            self.election_interval, self.state.to_follower
        )
        self.vote_count = 0
    ...
    def start(self):
        ...
        self.storage.update(
            {"term": self.storage.term + 1, "voted_for": self.id}
        )
    ...
    def on_receive_request_vote_response(self, data):
        ...
        If the vote was granted then check if we got majority and may become
        Leader
        """
...
class Follower(BaseState):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.election_timer = Timer(
            self.election_interval, self.start_election
        )
    ...
    def on_receive_append_entries(self, data):
        ...
        try:
            if self.log[new_index]["term"] != data["entries"][0]["term"]:
                ...
                # Ensure that matching entries are not erased and appended
                # again
                is_append = False
        except IndexError:
            pass
        ...
        if self.log.commit_index < data["commit_index"]:
            self.log.commit_index = min(
                data["commit_index"], self.log.last_log_index
            )

        # Respond True since entry matching prev_log_index and prev_log_term
        # was found
        response = {
            "type": "append_entries_response",
            "term": self.storage.term,
    ...
    @validate_term
    def on_receive_request_vote(self, data):
        if self.storage.voted_for is None and not data["type"].endswith(
            "_response"
        ):
            # Candidates' log has to be up-to-date

            # If the logs have last entries with different terms,
            # then the log with the later term is more up-to-date. If the logs
            # end with the same term, then whichever log is longer is more
            # up-to-date.
...
class State:
    """Abstraction layer between Server & Raft State and Storage/Log & Raft
    State"""
    ...
    def set_leader(self, leader):
        ...
        if (
            ...
            and (
                cls.wait_until_leader_future
                and not cls.wait_until_leader_future.done()
            )
    ...
    async def wait_until_leader(cls, node_id):
        """Await this function if you want to do nothing until node_id becomes
        a leader"""
        if node_id is None:
